...
BOOL CDpNumCtrl::DpSetData0(double dValue1, double dValue2, int type)
{
    ...
    case 0:    //* Refractive Power
        ...
        m_iIntMaxMinus = 25;
        break;
    case 1:    //* Spherical refractive power
        //* dValue2 : Current cylindrical refractive power
        //* Round to the third decimal place
        ...
        break;
    case 2:    //* Cylindrical refractive power
        //* dValue2 : Current cylindrical refractive power
        //* Round to the third decimal place
        dValue2 = _wtof(str_cvt(dValue2 * 1000.0, 0)) / 1000.0;
        if(dValue2 <= 0.0) {
    ...
    double dDmy = fabs(m_dNumValue) + KK_CEIL; //* 20251112 (Ver.2.27.0) Fixed a bug where numerical data such as 1.54999... was not rounded at the specified decimal places.
...
void CDpNumCtrl::DpCharacterCheck0(UINT nChar)
{
    ...
    case INT_SECOND_FIELD_START:
        //
        if ((nChar < 48) || (nChar > 51)) return;   //* The values that can be used in the tens place are 0, 1, 2.
        str[0] = (char) nChar;
        str[1] = '\0';
        tmpnum = _wtoi(str);
        //
...
void CDpNumCtrl::DpKeyFetch0(UINT nChar)
{
    ...
    case VK_UP:
        ...
        case INT_FIRST_FIELD_START:
            ...
            break;
        case FRAC_FIRST_FIELD_START:
            //* Shift + Arrow_UP: Counts in 0.10D
            if (GetKeyState(VK_SHIFT) < 0) {
                //* Shift + Ctrl + Arrow_UP: Counts in 0.10D
                if (GetKeyState(VK_CONTROL) < 0) {
                    //* FRAC_FIRST
                    //*   VK_SHIFT
                    //*   VK_CONTROL
                    //* Both are 0.10D steps, so skip this part.
                }
                if (m_strSign == "+")  {
                ...
                //* The normal 0.25D step goes here
                if (m_strSign == "+")  {
                    if (iFracTmp <25) {
                        m_iFracFirst = 2;
                        m_iFracSecond = 5;
                    ...
                    if (iFracTmp == 0 || (iFracTmp > 75 && iFracTmp <= 99)) {
                        m_iFracFirst = 7;
                        m_iFracSecond = 5;
                        m_iIntFirst--;
                    } else if (iFracTmp > 50 && iFracTmp <= 75) {
                        m_iFracFirst = 5;
                        m_iFracSecond = 0;
                    } else if (iFracTmp > 25 && iFracTmp <= 50) {
                        m_iFracFirst = 2;
                        m_iFracSecond = 5;
                    } else if (iFracTmp <= 25) {
                        m_iFracFirst = 0;
        ...
        case FRAC_SECOND_FIELD_START:
            if (GetKeyState(VK_SHIFT) < 0) {
                if (GetKeyState(VK_CONTROL) < 0) {
                    ...
                    //* Shift + Arrow_UP: Counts in 0.05D
                    if (m_strSign == "+")  {
                        if (m_iIntSecond * 1000 + m_iIntFirst * 100 + m_iFracFirst * 10 + m_iFracSecond < m_iIntMaxPlus * 100) {
                            m_iFracSecond = m_iFracSecond + 5;
                            if (m_iIntSecond * 1000 + m_iIntFirst * 100 + m_iFracFirst * 10 + m_iFracSecond > m_iIntMaxPlus * 100) {
                                m_iFracSecond = m_iFracSecond - 5;
                            }
                        }
                ...
                if (m_strSign == "+")  {
                    ...
                    if (iFracTmp == 0 || (iFracTmp > 75 && iFracTmp <= 99)) {   // こちらはOK
                        m_iFracFirst = 7;
                        m_iFracSecond = 5;
                        m_iIntFirst--;
                    } else if (iFracTmp > 50 && iFracTmp <= 75) {
    ...
    case VK_DOWN:
        ...
        case FRAC_FIRST_FIELD_START:
            ...
            if (GetKeyState(VK_SHIFT) < 0) {
                ...
                if (m_strSign == "+") {
                    if (iFracTmp >= 0 && iFracTmp < 25) {
                        m_iFracFirst = 7;
                        m_iFracSecond = 5;
                        if (m_iIntFirst > 0) {
                            m_iIntFirst--;
                        } else if (m_iIntFirst == 0){
                            m_strSign = "-";
                            m_iFracFirst = 2;
                            m_iFracSecond = 5;
                        }
                        ...
                        m_iFracFirst = 2;
                        m_iFracSecond = 5;
                    } else if (iFracTmp >= 75 && iFracTmp < 100) {
                        m_iFracFirst = 5;
                        m_iFracSecond = 0;
                    }
                } else {
        ...
        case FRAC_SECOND_FIELD_START:
            if (GetKeyState(VK_SHIFT) < 0) {
                if (GetKeyState(VK_CONTROL) < 0) {
                    ...
                    //* Shift + Ctrl + Arrow_UP: Counts in 0.05D
                    if (m_strSign == "+")  {
                        if (m_iIntSecond * 1000 + m_iIntFirst * 100 + m_iFracFirst * 10 + m_iFracSecond > 0) {
                            m_iFracSecond =  m_iFracSecond - 5;
                            if(m_iFracSecond < 0) {

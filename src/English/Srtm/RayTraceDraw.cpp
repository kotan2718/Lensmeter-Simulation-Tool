...
BOOL CRayTrace::GLS1()
{
    ...
    //*
    //* Calculate and plot z given the y,x data of a surface
    //*
    ...
    //* --------------------------------------
    //* Character set for the pint glass
    m_dFunc->P_Char_Set("Focusing Glass", "Times New Roman", 12, 68, -13, RGB(0, 0, 0));
    ...
    //*
    //* Storage of boundary surface distance
    //*

    //* m_dD_DefaultLengthAll: Distance from the edge of the collimator to the edge of the objective lens
    //*                        (excluding the thickness of the tested lens)
    //* Already calculated in DataIn()
...
{
    //*
    //* Drawing the target and the reference position of the pint glass
    //*
    long lCol;
    int nAXY = 0;
    ...
    //*
    //* Drawing of target position settings
    //*
    m_iIDFlg = 1;
    ...
    //* Drawing lenses
    //*
    //* tested lens and objective lens
    ...
    case 11:
    {
        for(int iNyu = m_iKY2 - 1; iNyu >= 5; iNyu--) {
            ...
            case 5:
                m_dLengthL = 20.0;
                //* m_dLengthL = 25.0;    // 20230701_This is too large. If the collimator and objective lens are removed, will it be possible to draw?
                m_dDeltaL = 4.0;
                m_dDK = m_dDKsub[iNyu];
                break;
            case 6:
...
BOOL CRayTrace::GLS_Kyokumen_Select(int iNyu)
{
    ...
    //* Selection of the surface shape
    m_iAsphericFlg = 0;
...
BOOL CRayTrace::GLSSUB(int iNyu, int nJKK, long lCol)
{
    ...
    if (m_iAsphericFlg == 0) {
        //* Drawing of coaxial and spherical lens
    ...
    if(m_iAsphericFlg == 1) {
        //*
        //* Drawing of aspherical or non-coaxial lens
        //*
        if(m_iASP == 1) {
            //*
            //* Drawing of toric lens
            ...
            //* Processing when the optical axis is shifted
            if(m_nSHAX[iNyu] == 1)    AxShift(iNyu);

            //* Processing when an eccentricity occurs
        ...
        if(m_iASP == 2) {
            //* Drawing of a rotationally symmetric aspheric lens
            ...
            if(m_nAS[iNyu] == 1) {
                //* When aspherical coefficients are only even orders
                dVH1 = 1.0;
                dZSum = 0.0;
                ...
                //* When the aspherical coefficients include odd orders
                dHVAL = sqrt(dRYRX);
                dVH1 = dHVAL;
                dZSum = 0.0;
...
void CRayTrace::MakeCorona(int nIKK, int nJKK)
{
    if (nIKK == 0 && nJKK == 0) {
        //* 2002.06.06
        //* Research the relationship between the degree of blurring of the corona image and the amount of prism.
        //* If " / 2.0" is not added, the prism quantity doubles.
...
{
    //* Drawing a corona image on a pint glass
    ...
    //* Performing the following calculation in "m_dPintGlassMemori = 180.0" causes failure in "sin(...)".
    //* A negative sign is applied even though the value is zero.
    //* It was discovered that in "KK_PI /180.0 * m_dPintGlassMemori",
    //* the result of the sin function differs when "m_dPintGlassMemori" is "0.0" versus when "m_dPintGlassMemori" is "180.0",
    //* and in "180.0", it becomes "-0.0000000000000014...".
    ...
    //* Prism level indicator: vertical direction
    dPriLine = 2.0;

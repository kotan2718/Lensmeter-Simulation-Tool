...
BOOL CDpNumCtrl::DpSetData1(double dValue, int type)
{
...
void CDpNumCtrl::DpCharacterCheck1(UINT nChar)
{
    ...
    switch (m_SelStart)
    {
        case INT_THIRD_FIELD_START:
            if ((nChar < 48) || (nChar > 50)) { //* The values ​​that can be used in the hundreds digit are 0 and 1.
                return;
            }
            str[0] = (char) nChar;
            str[1] = '\0';
...
{
    int ifTmp = m_iIntFirst * 10 + m_iFracFirst;
    switch (nChar) {
        case VK_HOME:
                ...
                case INT_FIRST_FIELD_START:
                    if (GetKeyState(VK_SHIFT) < 0) {
                        ...
                        if (ifTmp <25) {
                            ...
                            if (m_iIntThird * 100 + m_iIntSecond * 10 + m_iIntFirst > 177) {
                                m_iIntFirst = 0;
                                m_iIntSecond = 0;
                                m_iIntThird = 0;
                            }
            ...
            switch (m_SelStart) {
                ...
                case INT_FIRST_FIELD_START:
                    if (GetKeyState(VK_SHIFT) < 0) {
                        ...
                        //* Arrow_DOWN : Counts at 2.5°
                        if (ifTmp > 0 && ifTmp <= 25) {
                            m_iIntFirst = 0;
                            m_iFracFirst = 0;
                        } else if (ifTmp > 25 && ifTmp <= 50) {
                            m_iIntFirst = 2;
                            m_iFracFirst = 5;
                        } else if (ifTmp > 50 && ifTmp <= 75) {
                            m_iIntFirst = 5;
                            m_iFracFirst = 0;
                        } else if (ifTmp > 75 && ifTmp <= 100) {
                            m_iIntFirst = 7;
                            m_iFracFirst = 5;
                        } else if (ifTmp == 0) {
                            m_iIntFirst = 7;

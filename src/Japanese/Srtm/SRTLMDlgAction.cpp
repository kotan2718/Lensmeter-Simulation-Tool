...
BOOL CSRTLMDlg::R_KINJIKU()
{
    ...
    //* スクリーン位置からターゲットの位置を逆追跡（近軸）
    //
    //
    //* ----------------------------------------------------------------------
    //* 追跡する面が球面の場合はm_iASP に 0 が，
...
BOOL CSRTLMDlg::R_KINJIKU_Debug()
{
    ...
    //* R_KINJIK で求めたターゲットの位置が正しいかどうかの検算
    //
    //* コリメータ2検算
    //* m_iKY2 = 11;
    //* m_dD[6] = -1.79769313486232E+307;

//*    dImage1 = m_dD[6]; //* コリメータ2検算
//*    dImage1 = -27.8040783108784;
    dImage1 = m_dObjPoint1;

    int j;
//*    for(j = 8; j < m_iKY2; j++) //* コリメータ2検算
    for(j = 1; j < m_iKY2; j++) {
//*        if(j > 9)    dImage1 = dImage1 - m_dD[j - 1]; //* コリメータ2検算
...
BOOL CSRTLMDlg::Make_Lens()
{
    ...
    //* 被検レンズ C(5)__AN(6)_D(5)__C(6) の中心厚 m_dD(5) に応じて，
    //* 集光レンズ1 C(6)__C(7) までの距離 m_dD(6)を変え，
    //* C(6):被検レンズ裏面 から 集光レンズ第1面:C(7) までの距離を一定に保つ
...
{
    //* 後面が凸の場合に対応していなかったパターンが見つかったので修正(したつもり)。 2023/05/09
    //* 以下は暫定的なものなので、全体的な分岐については見直しが必要かも。

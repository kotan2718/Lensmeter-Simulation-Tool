...
{
    //
    //* sub functione CONVERT
    //* CONVERT as convert pi to pi-wave coordinate or pi-wave to pi coordinate
    //
...
BOOL CRayTrace::CTM(double dVEZ, double dVEY, double dVEX)
{
    ...
    //*
    //* 偏心によって新たな座標系π-waveの光軸ベクトルｅzが旧座標系πで(vz,vy,vx)
    //* として与えられたとき，ｅy,ｅxを計算し，座標変換行列を作る．
    //*
    //* mother program   :HS
    //* daughter program :
    //*
...
BOOL CRayTrace::HS(int iNyu)
{
    ...
    if(m_iIDFlg == 1) {
        //* sub function GLSSUB が参照する routine
        //* レンズの描画で座標系の変換があった場合に呼ばれる

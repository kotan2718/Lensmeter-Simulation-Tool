...
int CRayTrace::SkewRayTrace(int IKK)
{
    ...
    for (int iNyu = 1; iNyu <= m_iKY2; iNyu++) {
        ...
        if (iNyu == 1) {
            if (!RAYCHECK()) {
                return WNG_RAY_HEIGHT_OVER;        //* 200-> 追跡している光線が有効径から外れた
            }
        }
        //*
        //* ----------------------------------------------------------------------
        //* 追跡する面が球面で共軸の場合は         m_iAsphericFlg に 0 が，
        //*             非球面または非共軸の場合は m_iAsphericFlg に 1 が格納される
        //*     非球面の場合は回転対称    の場合は m_iASP に 2 が
        //*                   トーリック面の場合は m_iASP に 1 が格納される
        //*
        m_iAsphericFlg = 0;
        ...
        //* ------------------------------------------------------------------------
        //* 光線追跡の開始位置の設定
        //* 一般には入射瞳平面が開始位置になるが，このプログラムでは第１面の頂点を通
        //* る光軸との直交面，或は任意位置を開始位置に設定している
        //*
        if(iNyu == 1) {
            dDIM1 = -m_dQNTP;
        }
        ...
        //* --------------------------------------------------------------------------
        //* 追跡面が共軸かつ球面か，非共軸またはアスフェリック面かトーリック面かで場合
        //* 分けをする
        //*
        ...
        if(m_iAsphericFlg == 0) {
            if (!SRTSph(iNyu)) {
                return ERR_RAY_TRACE_SPH;        //* error code: 110 <- 1: 追跡不可能, 1: 共軸かつ球面, 0: SRTSph
            }
        }
        //* --------------------------------------------------------------------
        ...
        if(m_iAsphericFlg == 1) {
            ...
            if (m_iASP == 1) {
                if (!SRTToric(iNyu)) {
                    return ERR_RAY_TRACE_TORIC;  //* error code: 121 <- 1: 追跡不可能, 2: 非球面或は非光軸, 1: SRTToric
                }
            }
            //*
            //* 回転対称非球面の追跡
            ...
            if (m_iASP == 2) {
                if (!SRTAsph(iNyu)) {
                    return ERR_RAY_TRACE_ASPH;   //* error code: 122 <- 1: 追跡不可能, 2: 非球面或は非光軸, 2: SRTAsph
                }
            }
        ...
        if (dD12 < 0.0) {
            return ERR_RAY_TRACE;                //* error code: 100 <- 1: 追跡不可能, 0: 特定なし, 0: SkewRayTrace
        }
